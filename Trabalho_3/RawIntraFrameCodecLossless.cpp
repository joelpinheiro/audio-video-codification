...
RawIntraFrameCodecLossless::RawIntraFrameCodecLossless(char *inputFilePath, char *outputFilePath) {
    this->inputFilePath = inputFilePath;
    this->outputFilepath = outputFilePath;
}

void RawIntraFrameCodecLossless::intraFrameEncode(int predictorMode, int showVideo, int m) {
...
Mat *RawIntraFrameCodecLossless::resizeFrame(Mat *matToResize) {
    int pInit;
...
void RawIntraFrameCodecLossless::predictMat(int predictorMode) {
    if (predictorMode == 1) {
        // a
...
void RawIntraFrameCodecLossless::intraFrameDecode(int predictorMode, int showVideo) {
    GolombDecoder g(this->inputFilePath);
    ofstream output(this->outputFilepath,  ios::trunc | ios::binary);

    unsigned int m, frameCount, frameResolution_V, frameResolution_H, format, frameRate;
    ...
    frameCount = (unsigned)stoul(tokens[1]);
    frameResolution_V = (unsigned)stoul(tokens[2]);
    frameResolution_H = (unsigned)stoul(tokens[3]);
    format = (unsigned)stoul(tokens[4]);
    frameRate = (unsigned)stoul(tokens[5]);
    ...
    unsigned long long totalAmountOfInts = frameCount * (frameResolution_V*frameResolution_H + 2*((frameResolution_V/2) *
            (frameResolution_H/2)));
    unsigned long long amountOfFrames = frameCount;
    ...
    int x = 0;
    int z = 0;
    for(unsigned long long i = 0; i < amountOfFrames + 1; i++){
...
//            }
        ...
        for(int i = 0 ; i < frameResolution_V ; i++){
            for(int j = 0 ; j < frameResolution_H ; j++){
                ycrcb_merged.at<Vec3b>(i,j).val[0] = (int) yyMat->at<uchar>(i, j);
                ycrcb_merged.at<Vec3b>(i,j).val[1] = (int )uuMat->at<uchar>(i/2, j/2);
                ycrcb_merged.at<Vec3b>(i,j).val[2] = (int) vvMat->at<uchar>(i/2, j/2);
            }
        }
...
short RawIntraFrameCodecLossless::predictValue(int predictorMode, Mat *xxMat, int i, int j) {
